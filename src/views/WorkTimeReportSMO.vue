<template>
    <v-container fluid>
        ...
        <v-row>
            ...
            <v-col cols="3">
                <v-autocomplete :items="employeeFromAPI" dense v-model="emplToReport"
                                label="Сотрудник"
            ...
            <v-col cols="3" class="text-center">
                <v-btn color="success" :disabled="disableAddEmployeeButton || disableEditMode"
                       small
        ...
        methods: {
            ...
            getData: async function () {
                if (this.reportID !== null && +(this.reportID) !== 0) {
                    try {
                        ...
                        console.log('qs report', `${this.$store.getters.bxapi}pcs.reports.get?id=${this.reportID}&mode=full&userMode=${this.mode}&type=timeReport`)
            ...
            getUserMode: async function () {
                try {
                    ...
                    console.log('getUserMode user', this.userID)
                    this.mode = r.data.result
                    console.log('getUserMode = mode', r.data.result)
                } catch (e) {
                    console.log('get Reports err', e)
                }
            },
            ...
            if (window.BX24) {
                ...
                BX24.init(async function () {
                    BX24.callMethod('user.current', {}, async function (res) {
                        ...
                        console.log('CurrentReportID', self.reportID)
