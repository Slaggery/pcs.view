<template>
    <v-container fluid>
        ...
        <h2>Отчет по внутренним затратам на проект</h2>
        <v-row v-if="mode !== 'smo'">
            <v-col cols="3">
        ...
        <v-row>
            <v-col cols="12">
                ...
                >
                    ...
                    <template v-slot:item.actions="{item}">
                        <v-btn small icon @click="deleteItem(item)" v-if="mode !== 'smo'" :disabled="disableEditMode">
            ...
            <v-col cols="12" v-if="mode === 'smo'" class="text-center">
                <v-btn color="success" class="mr-2" :disabled="disableBossEditMode" @click="agree">Согласовать</v-btn>
                <v-btn color="error" class="ml-2" :disabled="disableBossEditMode" @click="returnReport">Вернуть на доработку</v-btn>
            </v-col>
    ...
    export default {
        ...
        computed: {
            ...
            computedHeaders() {
                let headers = this.headers.map(hdr => {
                    if (this.mode === 'smo' && hdr.value !== 'actions') {
                        return hdr
                    } else {
                        return hdr
                    }
        ...
        methods: {
            ...
            getSpendTypes: async function () {
                try {
                    const r = await this.$http.get(`${this.$store.getters.bxapi}pcs.spendtypes.list`)
